# bot.py — aiogram 3.7+ webhook bot for Telegram groups
import os
import asyncio
import logging
import aiosqlite
...
if not BASE_URL:
    raise SystemExit("Missing BASE_URL environment variable (your https Render URL)")
...
def build_mention_text(row, style: str, emoji: str) -> str:
    uid = row["user_id"]
    handle = f'@{row["username"]}' if row["username"] else ""
    fullname = " ".join(x for x in [row["first_name"], row["last_name"]] if x).strip() or "member"
    invisible = f'<a href="tg://user?id={uid}">\u2063</a>'  # invisible link = ping
    if style == "emoji":
        return f"{emoji} {handle or fullname}{invisible}"
    if style == "name":
        return f"{fullname}{invisible}"
    return f"{handle}{invisible}" if handle else invisible

def chunkify(seq: Iterable[str], n: int):
    buf: list[str] = []
    for x in seq:
        buf.append(x)
        if len(buf) >= n:
...
async def tag_all(msg: Message):
    ...
    if not rows:
        ...
        await msg.reply(
            "I don’t know anyone here yet. Use /rollcall so people can tap the button, or ask everyone to send one short message.",
            reply_markup=kb,
        )
        return
...
def build_app() -> web.Application:
    ...
    # Health endpoints (handy in Render)
    async def health(_):
    ...
    # Lifecycle: set/delete webhook and init DB
    ...
    setup_application(app, dp, bot=bot)  # required for aiogram 3
    return app
